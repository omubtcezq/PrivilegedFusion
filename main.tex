...
\begin{figure}[htbp]
  ...
  \label{fig:mse_privs}
\end{figure}
To demonstrate the effect of choosing parameters $\Sigma_z$ and $\Sigma_y$ (and therefore $\mat{Z}$ and $\mat{Y}$), figure \ref{fig:mse_params} considers shows how average error is affected when considering only a fixed privilege $p=2$. Here, it can be seen that $\Sigma_z$ has a larger effect on the difference between $\mathsf{e}^{[0,4]}$ and $\mathsf{e}^{[p,p]}$ than $\mathsf{e}^{[p,p]}$ and $\mathsf{e}^{[p,4]}$ while $\Sigma_y$ has the opposite. However, it can also be observed that both parameters affect both differences to a degree, revealing some the limitations of the proposed scheme.
\begin{figure}[htbp]
  \centering
  \includegraphics{figures/mse_params.pdf}
  \caption{Average error of $1000$ simulation runs for varying $\Sigma_z$ and $\Sigma_y$.}
  \label{fig:mse_params}
\end{figure}
Figure \ref{fig:trace_params}
...
\begin{figure}[htbp]
  ...
  \caption{Steady state trace of estiators when .}
