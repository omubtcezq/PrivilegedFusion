...
The idea behind our privileged estimation fusion scheme is to add correlated Gaussian keystreams to the measurements from each sensor. The noises can be removed by estimators holding respective sensor keys, while the correlation limits the additional information from fusing unprivileged measurements. As we assume that sensors are synchronised, we can capture the correlation between the modified measurements exactly by considering the stacked measurement model at time $k$, for an estimator with access to $q\leq N$ measurements, as
\begin{equation}
  \vec{y}_k^{(q)} = \mat{H}_k^{(q)}\vec{x}_k + \vec{v}_k^{(q)}
\end{equation}
where $\vec{v}_k^{(q)}\sim\mathcal{N}(\vec{0},\mat{R}^{(q)})$, with
\begin{equation*}
  \vec{y}_k^{(q)}=
  \begin{bmatrix}
    \vec{y}_{k,1}\\
...
\begin{equation*}
  \mat{R}^{(q)}=\mathsf{diag}\left(
    ...
    \end{bmatrix}\right)\,.
\end{equation*}
With measurement equations in this form, we can introduce the means of modifying measurements to affect estimation performance.
...
\begin{equation}
  ...
  \end{bmatrix}\right)\,.
\end{equation}
The generation of all Gaussian noises at timestep $k$, can then be computed by
\begin{equation}
  \begin{bmatrix}
    \vec{g}_{k,1}\\
    \vdots\\
  ...
  \end{bmatrix}\,,
\end{equation}
where each $\vec{z}_{k,i}$ is computed as $\vec{z}_k$ in \eqref{eqn:standard_gaussian_generation} using cipher key $\mathsf{sk}_i$ and $\mat{S}^\frac{1}{2}$ is a matrix such that $\mat{S}^\frac{1}{2}\mat{S}^{\frac{1}{2}\top}=\mat{S}$. The noises $\vec{g}_{k,i}$ can then be added to measurements from each sensor $i$, at timestep $k$, resulting in the modified measurement equations
\begin{equation}
\begin{split}
