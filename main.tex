...
First we consider the lower bound to the difference in estimation between estimators of the form $\mathsf{e}^{[0,N]}$ and $\mathsf{e}^{[p,p]}$. The observed measurement models for the two estimators are given by \eqref{eqn:0q_obs_measurement_model} and \eqref{eqn:pp_obs_measurement_model}, respectively, when $q=N$. 

% Scheme in terms of two algorithms here? (different from dot points)

With these measurement models and the system model \eqref{eqn:system_model}, the KF can be used to recursively compute the optimal estimate covariances $\mat{P}_k$ at each timestep $k$ for an estimator given an initial estimate covairance. Since the KF also preserves initial covariance order,
\begin{equation}
  \mat{P}_k \succeq \mat{P}_k^\prime \implies \mat{P}_{k+1} \succeq \mat{P}_{k+1}^\prime\,,
\end{equation}
we can compute a lower-bound for all possible covariances when letting $\mat{P}_0 = \mat{0}$.
